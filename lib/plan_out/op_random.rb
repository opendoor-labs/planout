...
module PlanOut
  ...
  class Sample < OpRandom
    def simple_execute
      param_choices = @parameters[:choices]
      choices = Array.new(param_choices)

      if @parameters.key? :draws
        num_draws = @parameters[:draws]

        raise "invalid choice" unless num_draws <= choices.length  
      else
